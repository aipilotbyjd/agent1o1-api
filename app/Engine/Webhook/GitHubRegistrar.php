...
class GitHubRegistrar implements WebhookRegistrar
{
    ...
    public function register(string $callbackUrl, array $events, array $credentials, array $providerConfig = []): array
    {
        ...
        if ($response->status() === 422) {
            // A webhook already exists for this URL. Delete it and re-create with our
            // secret so signature verification is consistent. If we can't find it, throw.
            return $this->replaceExistingWebhook($callbackUrl, $secret, $credentials, $providerConfig);
        }
    ...
    {
        return "/repos/{$providerConfig['owner']}/{$providerConfig['repo']}";
    }
     ...
     */
    private function replaceExistingWebhook(string $callbackUrl, string $secret, array $credentials, array $providerConfig): array
    {
        $response = $this->client($credentials)
        ...
        foreach ($response->json() as $hook) {
            if (($hook['config']['url'] ?? null) === $callbackUrl) {
                // Delete the stale webhook so we can re-create it with our secret
                $this->client($credentials)
                    ->delete($this->repoUrl($providerConfig)."/hooks/{$hook['id']}")
                    ->throw();

                // Re-create with the new secret
                return $this->register($callbackUrl, $hook['events'] ?? [], $credentials, $providerConfig);
            }
        }
